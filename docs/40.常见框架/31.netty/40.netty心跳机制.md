...
```makedown
Copynet.ipv4.tcp_keepalive_time = 7200
...
![image-20230707202759192](https%253A%252F%252F2290653824-github-io.oss-cn-hangzhou.aliyuncs.com%252Fimage-20230707202759192.png)
...
```java
Copypublic IdleStateHandler(long readerIdleTime, long writerIdleTime, 
...
```java
Copypackage com.rickiyang.learn.keepAlive;
...
```java
Copypublic class IdleStateHandler extends ChannelDuplexHandler {
...
```java
Copyprivate void initialize(ChannelHandlerContext ctx) {
...
```java
Copyprivate final class ReaderIdleTimeoutTask implements Runnable {
...
```txt
CopynextDelay = nextDelay - 当前时间 - 上次读取时间
```
...
```java
Copyprivate final class WriterIdleTimeoutTask implements Runnable {
...
```java
CopynextDelay = writerIdleTimeNanos - (System.nanoTime() - lastWriteTime)
```
