...
> 上下文切换（Context Switching）和用户内核态空间转换（User-Kernel Mode Switching）是两个概念，它们在操作系统中扮演不同的角色，但可能会相互影响。
>
> 1. 上下文切换（Context Switching）： 上下文切换是指从一个正在运行的进程或线程切换到另一个进程或线程的过程。每个进程或线程都有一个称为上下文（Context）的状态，包括寄存器的内容、程序计数器（PC）、内存映射、打开的文件等。当操作系统决定切换到另一个进程或线程时，它会保存当前进程或线程的上下文，并加载下一个进程或线程的上下文，使其能够继续执行。
>
>    上下文切换是多任务操作系统的核心机制，它使得多个进程或线程可以在同一时间共享处理器资源。但上下文切换是有代价的，因为它涉及到保存和加载大量的状态信息，可能会导致性能损失。
>
> 2. 用户内核态空间转换（User-Kernel Mode Switching）： 用户内核态空间转换是指在用户态（User Mode）和内核态（Kernel Mode）之间进行切换的过程。用户态是应用程序执行的环境，它的权限受限，不能直接访问底层硬件或执行特权指令。而内核态是操作系统内核执行的环境，它具有更高的权限，可以访问系统的所有资源和执行特权指令。
>
>    当应用程序需要执行需要更高权限的操作时，如进行系统调用或处理异常，就需要从用户态切换到内核态。相反，当操作系统完成所需任务后，将从内核态切换回用户态，让应用程序继续执行。
>
>    这种用户内核态空间转换通常伴随着上下文切换。因为当从用户态切换到内核态时，需要保存用户态的上下文，加载内核态的上下文；而当从内核态切换回用户态时，需要保存内核态的上下文，加载用户态的上下文。
>
> 总结： 上下文切换和用户内核态空间转换是两个不同的概念，但它们之间可能存在关联。上下文切换是在多任务操作系统中，从一个进程或线程切换到另一个的过程。用户内核态空间转换是在应用程序和操作系统内核之间切换权限级别的过程。在执行用户内核态空间转换时，可能会触发上下文切换，因为需要保存和加载不同权限级别的上下文信息
